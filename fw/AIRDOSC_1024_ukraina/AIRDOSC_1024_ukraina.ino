...
String FWversion = "PS_UA_1024_v1"; // Output data format
...
uint8_t histogram[128];       // buffer for histogram
...
*/  
  for(uint16_t i=0; i<(GPSdelay); i++)  // measurements between GPS aquisition
  {
    ...
    for (uint32_t i=0; i<44643; i++)    // cca 5 s
...
*/            
      ...
      display.setTextSize(2);      // Normal 1:1 pixel scale
      display.setCursor(0, 0);     // Start at top-left corner
      display.print(String((float)flux/5,1) + " CPS");
      ...
      {
        display.drawLine(i,61-histogram[i], i, 61, WHITE);
      }
